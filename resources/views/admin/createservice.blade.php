...
<body class="bg-[#5A7684] flex flex-col min-h-screen w-full">
    ...
    <div class="flex-grow flex items-center justify-center w-full">
        <div class="w-full max-w-5xl">
            <h1 class="text-center text-white text-3xl mb-8">Добавить услугу</h1>
            <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" class="w-full">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block text-white">Название</label>
                    <input type="text" name="name" id="name" class="w-full bg-transparent border border-white text-white px-4 py-2 rounded">
                </div>
                <div class="mb-4">
                    <label for="description" class="block text-white">Описание</label>
                    <textarea name="description" id="description" class="w-full bg-transparent border border-white text-white px-4 py-2 rounded"></textarea>
                </div>
                <div class="mb-4">
                    <label for="price" class="block text-white">Цена</label>
                    <input type="text" name="price" id="price" class="w-full bg-transparent border border-white text-white px-4 py-2 rounded">
                </div>
                <div class="mb-4">
